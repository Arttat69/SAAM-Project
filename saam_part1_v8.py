...
def with_unlocked_fallback(target_path: str, write_fn):
    """
    Run a write operation and fall back to a timestamped filename when
    Windows denies access because the destination file is currently open.
    """
    try:
        ...
        p = Path(target_path)
        fallback = str(p.with_name(f"{p.stem}_{datetime.now():%Y%m%d_%H%M%S}{p.suffix}"))
        write_fn(fallback)
        print(
            f"[warning] Could not overwrite locked file '{target_path}'. "
            f"Wrote output to '{fallback}' instead."
        )
        return fallback
...
def ledoit_wolf_cc(X: np.ndarray):
    """
    Ledoit-Wolf analytical shrinkage towards the constant-correlation target.
    X must be demeaned, shape (T, N) — T observations, N assets.
    Returns (Sigma_shrunk, delta, r_bar).
    """
    T, N = X.shape
    S = X.T @ X / T
    var = np.diag(S)
...
def solve_min_variance(Sigma, verbose=True):
    """
    Long-only min-var: min a'Σa  s.t. sum(a)=1, a≥0.

    Teacher check (Issue 1 — scaling convergence):
    ------------------------------------------------
    The objective w'Σw uses monthly covariances (~1e-4 magnitude). SLSQP's
    default tolerances can terminate early when the objective is tiny. We
    therefore re-run the optimisation with the objective multiplied by
    increasing scale factors (1, 100, 1000, 10000) and verify that the
    returned weights are stable across scales. Convergence is declared when
    the maximum absolute weight change between two consecutive scales is
    below 1e-4. If the weights shift materially, the optimizer was not
    converging correctly at the raw scale.

    Note: the default argument s=scale in the lambda is mandatory to
    capture the loop variable by value rather than by reference (classic
    Python closure pitfall in loops).
    """
    n  = Sigma.shape[0]
    a0 = np.full(n, 1.0 / n)
    ...
    for scale in [1, 100, 1_000, 10_000]:
        res = minimize(
            lambda a, s=scale: s * float(a @ Sigma @ a),
            a0,
            method="SLSQP",
            bounds=[(0.0, 1.0)] * n,
        ...
        if not res.success:
            raise RuntimeError(f"SLSQP failed at scale={scale}: {res.message}")

        w_cand = np.where(res.x < 1e-10, 0.0, res.x)
        w_cand = w_cand / w_cand.sum()

        if prev_w is not None:
            max_diff = float(np.max(np.abs(w_cand - prev_w)))
            if verbose:
                print(f"    [scaling] scale={scale:>6}: "
                      f"max_w={w_cand.max():.4f}, diff_from_prev={max_diff:.2e}")
            if max_diff < 1e-4:
                if verbose:
                    print(f"    [scaling] -> converged at scale={scale}")
                w_final   = w_cand
                converged = True
                break
        else:
            if verbose:
                print(f"    [scaling] scale={scale:>6}: "
                      f"max_w={w_cand.max():.4f} (baseline)")

        prev_w  = w_cand
        w_final = w_cand          # keep last iterate in case loop ends without converging

    if not converged and verbose:
        print("    [scaling] WARNING: weights did not converge across scales — "
              "optimizer may not have found the true minimum. "
              "Consider increasing SLSQP_MAXITER or using a different solver.")
...
def export_part1_excel_template(template_path, output_path, stats, out_df):
    ...
    # ---------------------------------------------------------
    with_unlocked_fallback(output_path, lambda path: wb.save(path))
...
def main():
    ...
    for year_end in range(START_YEAR_OOS - 1, END_YEAR_OOS):   # 2013 … 2024
        ...
        Sigma_sample = Sigma.copy()

        # ── Issue 3: Covariance matrix quality audit ──────────────────────
        # Check (a) how many firms have sparse return history in the window,
        # and (b) whether the covariance matrix is well-conditioned.
        # A very high condition number (>> 1e6) or a near-zero minimum
        # eigenvalue indicates that the matrix is poorly estimated — typically
        # because many asset pairs share very few overlapping observations.
        # In that case, the optimizer may concentrate weight into a handful of
        # stocks to exploit numerical artefacts rather than true low variance.
        R_check    = ri_returns.loc[elig, cols]
        obs_frac   = R_check.notna().mean(axis=1)
        thin_firms = int((obs_frac < 0.5).sum())          # <50 % obs in window
        eigvals    = np.linalg.eigvalsh(Sigma)
        min_eig    = float(eigvals.min())
        cond_num   = float(eigvals.max() / max(min_eig, 1e-12))
        print(f"  [cov audit] firms<50%obs={thin_firms}/{len(elig)}, "
              f"min_eigval={min_eig:.2e}, cond={cond_num:.1e}")
        if cond_num > 1e8:
            print("  [cov audit] WARNING: very high condition number — "
                  "covariance matrix may be unreliable. "
                  "Consider Ledoit-Wolf shrinkage (USE_LEDOIT_WOLF=True).")
        # ── Apply Ledoit-Wolf shrinkage if enabled ─────────────────────────────
        if USE_LEDOIT_WOLF:
            R_mat = ri_returns.loc[elig, cols].fillna(0.0).to_numpy().T  # shape (T, N)
            R_dem = R_mat - R_mat.mean(axis=0, keepdims=True)
            Sigma, delta_lw, r_bar_lw = ledoit_wolf_cc(R_dem)
            Sigma = Sigma + 1e-8 * np.eye(len(elig))  # keep ridge for safety
            eigvals_lw = np.linalg.eigvalsh(Sigma)
            print(
                f"  [LW]  delta_hat={delta_lw:.4f}, r_bar={r_bar_lw:.4f}, "
                f"cond_after={eigvals_lw.max()/eigvals_lw.min():.1e}"
            )
        else:
            Sigma = Sigma_sample

        # ── Issue 1: Optimizer convergence (scaling check) ────────────────
        # solve_min_variance internally tests scales 1×, 100×, 1000×, 10000×
        # and prints whether the weights converge. If they do not, the
        # optimizer was terminating too early at the raw (tiny) objective scale.
        w = solve_min_variance(Sigma, verbose=True)

        portfolios[year_end] = {"isins": elig, "weights": w}
        print(f"Year {year_end}: eligible={len(elig)}, max_w={w.max():.4f}")

        # ── Issue 2: Concentration diagnostics (HHI + top holdings) ──────
        # The Herfindahl-Hirschman Index (HHI) = sum(w_i^2).
        # For a perfectly diversified equal-weight portfolio HHI = 1/N.
        # Values much larger than 1/N signal pathological concentration,
        # which may indicate a poorly conditioned covariance matrix or a
        # near-degenerate stock that dominates the minimum-variance solution.
        w_series = pd.Series(w, index=elig)
        hhi      = float(np.sum(w ** 2))
        n_eff    = 1.0 / hhi                              # effective number of stocks
        top3     = w_series.nlargest(3)
        top3_str = ", ".join(f"{isin[-6:]}={v:.3f}" for isin, v in top3.items())
        print(f"  [concentration] HHI={hhi:.4f}, "
              f"N_eff={n_eff:.1f} (naive 1/N={1/len(elig):.4f}), "
              f"Top3: {top3_str}")
        if w.max() > 0.20:
            print(f"  [concentration] WARNING: single stock weight "
                  f"{w.max():.1%} exceeds 20%. "
                  f"Portfolio is highly concentrated — likely a covariance "
                  f"matrix quality issue (see cov audit above).")

 # ══════════════════════════════════════════════════════════════
    LAWSON = 'JP3982100004'

    # ── Check 1: -100% return in July 2024 ────────────────────────
    if LAWSON in ri_returns.index:
        jul_cols = [c for c in ri_returns.columns if str(c).startswith('2024-07')]
        if jul_cols:
            val = ri_returns.loc[LAWSON, jul_cols[0]]
            print(f"\n[LAWSON AUDIT] Return July 2024 : {val}")
            if val == -1.0:
                print("  ✅ -100% return correctly booked")
            elif pd.isna(val):
                print("  ❌ NaN — delist return NOT booked (cumulative perf overstated)")
            else:
                print(f"  ⚠️  Unexpected value: {val:.4f} — check delist logic")
        else:
            print("\n[LAWSON AUDIT] ⚠️ No July 2024 column found in ri_returns")
            print("  Columns around 2024:", [str(c) for c in ri_returns.columns if '2024' in str(c)][:6])
    else:
        print(f"\n[LAWSON AUDIT] ⚠️ {LAWSON} not found in ri_returns index at all")

    # ── Bonus: return series around delist date ───────────────────
    if LAWSON in ri_returns.index:
        window_cols_lawson = [c for c in ri_returns.columns if '2024-04' <= str(c) <= '2024-10']
        print(f"[LAWSON AUDIT] Monthly returns Apr–Oct 2024:")
        print(ri_returns.loc[LAWSON, window_cols_lawson].to_string())

    # ── Check 2: Lawson absent from 2025 portfolio ────────────────
    # portfolios is keyed by year_end; year_end=2024 → holds during 2025
    if 2024 in portfolios:
        isins_2025 = portfolios[2024]['isins']
        weights_2025 = portfolios[2024]['weights']
        if LAWSON in isins_2025:
            idx = isins_2025.index(LAWSON)
            w_lawson = weights_2025[idx]
            print(f"\n[LAWSON AUDIT] Weight in 2025 portfolio: {w_lawson:.6f}  ❌ Still held after delist!")
        else:
            print(f"\n[LAWSON AUDIT] Lawson not in 2025 eligible set  ✅ Correctly excluded")
    else:
        print("\n[LAWSON AUDIT] No portfolio found for year_end=2024")

    # ── Bonus: show Lawson weight across all years ────────────────
    print("\n[LAWSON AUDIT] Lawson weights across all years:")
    for ye, info in sorted(portfolios.items()):
        isins = info['isins']
        if LAWSON in isins:
            idx = isins.index(LAWSON)
            w = info['weights'][idx]
            if w > 0:
                print(f"  Year {ye+1}: weight = {w:.4f}")
    # ═════════════════════════════════════════════════════════════
    ...
    with_unlocked_fallback(
        os.path.join(RESULTS_DIR, "part1_results.csv"),
        lambda path: out.to_csv(path, index=False),
    )
    ...
    with_unlocked_fallback(
        os.path.join(RESULTS_DIR, "part1_summary_statistics.csv"),
        lambda path: stats.to_csv(path),
    )
    ...
    with_unlocked_fallback(
        ...
        lambda path: pd.concat(comp, ignore_index=True).to_csv(path, index=False),
    )
