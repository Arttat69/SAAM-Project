"""
SAAM Project — Part 2 (v8)
Sections 3.1 | 3.2 | 3.3 | 3.4 | 4.1 | 4.2 — Group: Pacific / Scope 1

Changes vs v7 (fixes + additions)
──────────────────────────────────
FIX-1  Import from saam_part1_complete_v7.py  (was v6 — caused FileNotFoundError).
FIX-2  apply_delisting_to_returns() called with correct 4-arg signature
        (ri_prices_returns passed as price_df, matching Part 1 v7 exactly).
FIX-3  apply_terminal_loss_for_permanent_disappearances() now called after
        apply_delisting_to_returns(), mirroring the Part 1 v7 pipeline.
FIX-4  Dual RI price panels:
          ri_prices_universe  (preserve_december_missing=True)  → eligibility
          ri_prices_returns   (preserve_december_missing=False) → returns / Σ
        Aligned with Part 1 v7 to guarantee identical return series.
FIX-5  adjust_mv_caps_for_terminal_events() applied to VW benchmark caps,
        matching Part 1 v7 benchmark construction.
ADD-1  perf_stats_extended() replaces perf_stats() for all portfolio tables:
          VaR 95%  / VaR 99%          (historical monthly, reported as +loss)
          ES  95%  / ES  99%          (Expected Shortfall — coherent risk measure)
          Maximum Drawdown            (peak-to-trough, negative fraction)
ADD-2  perf_stats_relative() — benchmark-relative metrics for TE portfolios:
          Ex-post Active Return (annualised)
          Ex-post Tracking Error (annualised)
          Information Ratio
          Average Active Share (Lecture 5 slide 17)
ADD-3  all_portfolio_stats_extended.csv — all 5 portfolios × all metrics.
ADD-4  relative_stats_te_portfolios.csv — benchmark-relative table for §3.3 / §4.1.
ADD-5  stats_34_section_comparison.csv / stats_42_section_comparison.csv
        for direct copy-paste into the report.
"""

import os
import importlib.util
import warnings

import cvxpy as cp
import numpy as np
import pandas as pd

warnings.filterwarnings("ignore")

# ─────────────────────────────────────────────────────────────────────────────
# CONFIGURATION
# ─────────────────────────────────────────────────────────────────────────────
RESULTS_PART1   = "resultsPart1"
REGION_CODE     = "PAC"
START_YEAR_OOS  = 2014
END_YEAR_OOS    = 2025
DECISION_YEARS  = list(range(2013, 2025))
Y0              = 2013
THETA           = 0.10          # net-zero annual decarbonisation rate

WINDOW_YEARS    = 10
MIN_OBS_MONTHS  = 36
STALE_THRESHOLD = 0.50
LOW_PRICE_THRESHOLD = 0.5
RIDGE           = 1e-8

try:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
except NameError:
    BASE_DIR = os.getcwd()
DATA_DIR = os.path.join(BASE_DIR, "data")


def data_path(filename: str) -> str:
    """Resolve data file robustly across script/notebook working directories."""
    from pathlib import Path

    candidates = []
    for anchor in [Path(DATA_DIR), Path(BASE_DIR), Path.cwd()]:
        candidates.append(anchor / filename)
        candidates.append(anchor / "data" / filename)
        for parent in [anchor, *anchor.parents]:
            candidates.append(parent / filename)
            candidates.append(parent / "data" / filename)

    seen = set()
    for p in candidates:
        key = str(p.resolve()) if p.exists() else str(p)
        if key in seen:
            continue
        seen.add(key)
        if p.exists():
            return str(p)
    raise FileNotFoundError(f"Could not find required file: {filename}")


CO2_FILE = "DS_CO2_SCOPE_1_Y_2025.xlsx"
RF_FILE  = "Risk_Free_Rate_2025.xlsx"

USE_LEDOIT_WOLF = True
RESULTS_PART2   = "ResultsPart2_LW" if USE_LEDOIT_WOLF else "ResultsPart2"
os.makedirs(RESULTS_PART2, exist_ok=True)

# ─────────────────────────────────────────────────────────────────────────────
# FIX-1 — IMPORT HELPERS FROM PART 1 v7  (was saam_part1_complete_v6.py)
# ─────────────────────────────────────────────────────────────────────────────
_spec = importlib.util.spec_from_file_location(
    "part1", os.path.join(BASE_DIR, "saam_part1_complete_v7.py")
)
_p1 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_p1)

load_datastream_wide   = _p1.load_datastream_wide
load_annual_panel      = _p1.load_annual_panel
load_rf_monthly        = _p1.load_rf_monthly
parse_monthly_columns  = _p1.parse_monthly_columns
extract_delist_date    = _p1.extract_delist_date
clean_monthly_ri_prices = _p1.clean_monthly_ri_prices
apply_delisting_to_returns = _p1.apply_delisting_to_returns          # 4-arg version
apply_terminal_loss_for_permanent_disappearances = (                  # FIX-3
    _p1.apply_terminal_loss_for_permanent_disappearances)
adjust_mv_caps_for_terminal_events = _p1.adjust_mv_caps_for_terminal_events  # FIX-5
year_end_col         = _p1.year_end_col
window_cols          = _p1.window_cols
build_investment_set = _p1.build_investment_set
estimate_moments     = _p1.estimate_moments     # NaN-aware, ddof=0 (brief formula)
perf_stats           = _p1.perf_stats           # kept for template compatibility
print("Part 1 v7 functions imported successfully.")

# ─────────────────────────────────────────────────────────────────────────────
# EXTENDED PERFORMANCE STATISTICS  (ADD-1 / ADD-2)
# ─────────────────────────────────────────────────────────────────────────────

def perf_stats_extended(r: pd.Series, rf: pd.Series = None) -> dict:
    """
    Comprehensive ex-post performance statistics for the Part 2 final report.

    Standard metrics (template-aligned)
    ------------------------------------
    Annualized Average Return         12 × mean(r_m)
    Annualized Volatility             std(r_m) × sqrt(12)   [ddof=1]
    Annualized Cumulative Return      CAGR = (1+r)^(12/T) − 1
    Sharpe Ratio                      sqrt(12) × mean(excess) / std(excess)
    Minimum Monthly Return            min(r_m)
    Maximum Monthly Return            max(r_m)

    Risk metrics (Lecture 5, slides 43-56)
    ----------------------------------------
    VaR 95% (monthly)                 −5th-percentile of r_m  (positive loss)
    VaR 99% (monthly)                 −1st-percentile of r_m  (positive loss)
    ES  95% (monthly)                 E[−r | r ≤ −VaR95]      (positive loss)
    ES  99% (monthly)                 E[−r | r ≤ −VaR99]      (positive loss)
    Maximum Drawdown                  max peak-to-trough drop  (negative fraction)
    """
    r = r.dropna()
    T = len(r)
    if T == 0:
        keys = [
            "Annualized Average Return", "Annualized Volatility",
            "Annualized Cumulative Return", "Sharpe Ratio",
            "Minimum Monthly Return", "Maximum Monthly Return",
            "VaR_95 (monthly)", "VaR_99 (monthly)",
            "ES_95 (monthly)",  "ES_99 (monthly)", "Max_Drawdown",
        ]
        return {k: np.nan for k in keys}

    rf_aligned = (rf.reindex(r.index).fillna(0.0)
                  if rf is not None else pd.Series(0.0, index=r.index))
    excess  = r - rf_aligned
    mean_m  = r.mean()
    vol_m   = r.std(ddof=1)
    vol_exc = excess.std(ddof=1)

    # Historical VaR (positive loss = minus the return quantile)
    var95 = -float(np.percentile(r, 5))
    var99 = -float(np.percentile(r, 1))

    # Historical Expected Shortfall (average loss beyond VaR threshold)
    tail95 = r[r <= -var95]
    es95   = float(-tail95.mean()) if len(tail95) > 0 else var95
    tail99 = r[r <= -var99]
    es99   = float(-tail99.mean()) if len(tail99) > 0 else var99

    # Maximum Drawdown on compounded wealth path
    cum         = (1.0 + r).cumprod()
    running_max = cum.cummax()
    mdd         = float((cum / running_max - 1.0).min())

    return {
        "Annualized Average Return"   : 12.0 * mean_m,
        "Annualized Volatility"       : vol_m * np.sqrt(12.0),
        "Annualized Cumulative Return": (1.0 + r).prod() ** (12.0 / T) - 1.0,
        "Sharpe Ratio"                : (excess.mean() / vol_exc * np.sqrt(12.0)
                                         if vol_exc > 0 else np.nan),
        "Minimum Monthly Return"      : r.min(),
        "Maximum Monthly Return"      : r.max(),
        "VaR_95 (monthly)"           : var95,
        "VaR_99 (monthly)"           : var99,
        "ES_95 (monthly)"            : es95,
        "ES_99 (monthly)"            : es99,
        "Max_Drawdown"               : mdd,
    }


def perf_stats_relative(r_portfolio: pd.Series,
                        r_benchmark: pd.Series,
                        portfolio_weights: dict,
                        benchmark_weights: dict,
                        rf: pd.Series = None) -> dict:
    """
    Ex-post benchmark-relative performance statistics (Lecture 5, slides 15-17).

    Parameters
    ----------
    r_portfolio       Monthly returns of the active portfolio.
    r_benchmark       Monthly returns of the benchmark (VW).
    portfolio_weights dict keyed by DECISION_YEAR Y → {"isins": [...], "weights": array}
    benchmark_weights dict keyed by DECISION_YEAR Y → {"isins": [...], "weights": array}

    Returns
    -------
    Active Return (ann.)    Mean(R_p − R_b) × 12
    Tracking Error (ann.)   std(R_p − R_b) × sqrt(12)   [ddof=1]
    Information Ratio       Active Return / Tracking Error
    Avg Active Share        (1/2) Σ|w_p,i − w_b,i|  averaged across rebalancing years
                            (Lecture 5 slide 17)
    """
    r_p    = r_portfolio.dropna()
    r_b    = r_benchmark.reindex(r_p.index).dropna()
    common = r_p.index.intersection(r_b.index)
    r_p, r_b = r_p[common], r_b[common]
    active = r_p - r_b

    ann_active = 12.0 * active.mean()
    te_ann     = active.std(ddof=1) * np.sqrt(12.0)
    ir         = ann_active / te_ann if te_ann > 0 else np.nan

    # Active Share: average over all decision years where both weight dicts exist
    active_shares = []
    for Y in sorted(set(portfolio_weights) & set(benchmark_weights)):
        p_info = portfolio_weights[Y]
        b_info = benchmark_weights[Y]
        all_isins = list(set(p_info["isins"]) | set(b_info["isins"]))
        wp = (pd.Series(dict(zip(p_info["isins"], p_info["weights"])))
              .reindex(all_isins).fillna(0.0))
        wb = (pd.Series(dict(zip(b_info["isins"], b_info["weights"])))
              .reindex(all_isins).fillna(0.0))
        active_shares.append(0.5 * np.abs(wp.values - wb.values).sum())

    return {
        "Active Return (ann.)"  : ann_active,
        "Tracking Error (ann.)" : te_ann,
        "Information Ratio"     : ir,
        "Avg Active Share"      : float(np.mean(active_shares)) if active_shares else np.nan,
    }


# ─────────────────────────────────────────────────────────────────────────────
# HELPERS — carbon metrics, OOS returns
# ─────────────────────────────────────────────────────────────────────────────

def get_carbon_vectors(Y: int, isins: list):
    """Return (CO2_emissions, revenues_M_USD, annual_market_cap) for year Y."""
    e = (co2_ff[Y].reindex(isins).fillna(0.0).values
         if Y in co2_ff.columns else np.zeros(len(isins)))
    r = (revM_ff[Y].reindex(isins).fillna(np.nan).values
         if Y in revM_ff.columns else np.full(len(isins), np.nan))
    c = (capA_ff[Y].reindex(isins).fillna(0.0).values
         if Y in capA_ff.columns else np.zeros(len(isins)))
    return e, r, c


def carbon_intensity_vec(e, r):
    """CI_i = E_i / Rev_i  [tCO2 / M USD revenue]"""
    with np.errstate(invalid="ignore", divide="ignore"):
        return np.where(r > 0, e / r, np.nan)


def waci_metric(w, ci):
    """WACI = Σ_i α_i × CI_i  (re-normalised over firms with valid CI)."""
    valid = ~np.isnan(ci)
    if not valid.any():
        return np.nan
    w2 = np.where(valid, w, 0.0)
    s  = w2.sum()
    return float((w2 / s) @ np.where(valid, ci, 0.0)) if s > 0 else np.nan


def cf_metric(w, e, c):
    """Carbon Footprint  CF = Σ_i α_i × (E_i / Cap_i)  [tCO2 / M USD invested]."""
    with np.errstate(invalid="ignore", divide="ignore"):
        ratio = np.where(c > 0, e / c, 0.0)
    return float(w @ ratio)


def cf_vw_metric(e, c):
    """VW Carbon Footprint  CF_vw = Σ E_i / Σ Cap_i  [tCO2 / M USD invested]."""
    total = c.sum()
    return float(e.sum() / total) if total > 0 else np.nan


def e_over_c_vec(e, c):
    """Return E_i/Cap_i vector used as constraint coefficient in optimisation."""
    with np.errstate(invalid="ignore", divide="ignore"):
        return np.where(c > 0, e / c, 0.0)


def compute_oos_returns(portfolios_dict, ri_ret, dates, start_yr, end_yr):
    """
    OOS monthly returns with intra-year weight drift.

    Implements α_{i,t+k} = α_{i,t+k−1} × (1+R_{i,t+k}) / (1+R_{p,t+k})
    as specified in the project brief and Lecture 5 slide 35.
    Missing realized returns are filled with 0 (consistent with Part 1 pipeline
    after terminal-loss treatment; only zero-weight positions can be NaN).
    """
    out_r, out_d = [], []
    WEIGHT_TOL   = 1e-12
    for year_end in sorted(portfolios_dict.keys()):
        invest_year = year_end + 1
        if invest_year < start_yr or invest_year > end_yr:
            continue
        year_months = [d for d in dates if d.year == invest_year]
        if not year_months:
            continue
        isins = portfolios_dict[year_end]["isins"]
        w     = portfolios_dict[year_end]["weights"].copy()
        for d in year_months:
            r_i = (ri_ret.loc[isins, d].fillna(0.0).to_numpy()
                   if d in ri_ret.columns else np.zeros(len(isins)))
            r_p = float(w @ r_i)
            out_r.append(r_p)
            out_d.append(d)
            denom = 1.0 + r_p
            if denom > 0:
                w = w * (1.0 + r_i) / denom
                w = np.where(w < WEIGHT_TOL, 0.0, w)
                if w.sum() > 0:
                    w = w / w.sum()
    return pd.Series(out_r, index=pd.to_datetime(out_d)).sort_index()


def solve_cvxpy(Sigma_arr, e_c, cf_target, w_ref=None, mode="mv"):
    """
    Solve the constrained QP via CVXPY (falls back from OSQP to SCS).

    mode='mv' : min α′Σα          s.t. sum=1, α≥0, α·e_c ≤ cf_target
    mode='te' : min (α-w_ref)′Σ(α-w_ref)   same constraints
    """
    N  = Sigma_arr.shape[0]
    Sp = cp.psd_wrap(Sigma_arr)
    alpha = cp.Variable(N)
    obj = (cp.Minimize(cp.quad_form(alpha, Sp))
           if mode == "mv"
           else cp.Minimize(cp.quad_form(alpha - w_ref, Sp)))
    prob = cp.Problem(obj, [cp.sum(alpha) == 1, alpha >= 0,
                            alpha @ e_c <= cf_target])
    for solver in (cp.OSQP, cp.SCS):
        try:
            kw = ({"eps_abs": 1e-8, "eps_rel": 1e-8, "max_iter": 20_000}
                  if solver == cp.OSQP else {})
            prob.solve(solver=solver, **kw)
        except Exception:
            continue
        if (alpha.value is not None
                and prob.status in ("optimal", "optimal_inaccurate")):
            w = np.maximum(alpha.value, 0.0)
            s = w.sum()
            return w / s if s > 0 else np.ones(N) / N
    return None


# ─────────────────────────────────────────────────────────────────────────────
# LEDOIT-WOLF CONSTANT-CORRELATION SHRINKAGE  (Ledoit & Wolf, JPM 2004)
# Lecture 4, slides 41-42
# ─────────────────────────────────────────────────────────────────────────────
def ledoit_wolf_cc(X: np.ndarray):
    """
    Analytical Ledoit-Wolf shrinkage estimator, constant-correlation target.

    Shrinkage estimator: Σ̂ = δ̂·F + (1−δ̂)·S
    Target F (constant-correlation model, Ledoit & Wolf JPM 2004):
        f_ii = s_ii   (variances unchanged)
        f_ij = r̄ × σ_i × σ_j   for i ≠ j

    Optimal shrinkage intensity:  δ̂ = clip( (π̂ − ρ̂) / (T·γ̂),  0, 1 )
    where π̂, ρ̂, γ̂ are consistent estimators as derived in Lecture 4 slides 39-41.

    Parameters
    ----------
    X : ndarray of shape (T, N)  — demeaned monthly return matrix

    Returns
    -------
    Sigma_lw : (N, N) ndarray — shrinkage covariance matrix
    delta    : float          — optimal shrinkage intensity δ̂
    r_bar    : float          — average pairwise sample correlation r̄
    """
    T, N = X.shape
    S   = X.T @ X / T                               # sample covariance (ddof=0)
    var = np.diag(S)
    std = np.sqrt(np.maximum(var, 1e-16))
    std_outer = np.outer(std, std)

    # Average sample correlation r̄
    corr = S / std_outer
    np.fill_diagonal(corr, 0.0)
    r_bar = corr.sum() / (N * (N - 1))

    # Constant-correlation target F
    F = r_bar * std_outer
    np.fill_diagonal(F, var)

    # π̂ = Σ_ij (1/T) Σ_t [(R_it R_jt)² − s_ij²]
    X2     = X ** 2
    pi_mat = X2.T @ X2 / T - S ** 2
    pi_hat = pi_mat.sum()

    # ρ̂ — covariance between shrinkage target and sample entries
    rho_hat  = np.diag(pi_mat).sum()
    X3       = X ** 3
    Theta_II = X3.T @ X / T - S * var[:, None]
    Theta_JJ = X.T @ X3 / T - S * var[None, :]
    ratio_JI = std[None, :] / std[:, None]
    ratio_IJ = std[:, None] / std[None, :]
    rho_mat  = (r_bar / 2) * (ratio_JI * Theta_II + ratio_IJ * Theta_JJ)
    rho_hat += rho_mat.sum() - np.diag(rho_mat).sum()

    # γ̂ = ||F − S||²_F
    gamma_hat = np.sum((F - S) ** 2)

    kappa = (pi_hat - rho_hat) / gamma_hat if gamma_hat > 1e-30 else 0.0
    delta = float(np.clip(kappa / T, 0.0, 1.0))
    return delta * F + (1.0 - delta) * S, delta, r_bar


# ─────────────────────────────────────────────────────────────────────────────
# STEP 1 — RELOAD MONTHLY RI / MV DATA  (identical pipeline to Part 1 v7)
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Step 1 — Loading and cleaning monthly RI / MV data…")

static = pd.read_excel(data_path("Static_2025.xlsx"), engine="openpyxl")
static.columns = ["ISIN", "NAME", "Country", "Region"]
static["ISIN"] = static["ISIN"].astype(str).str.strip()
pac       = static[static["Region"] == REGION_CODE].set_index("ISIN")
pac_isins = set(pac.index)
print(f"  Pacific firms: {len(pac_isins)}")

# Annual RI — for delist date extraction
ri_y = load_datastream_wide(data_path("DS_RI_T_USD_Y_2025.xlsx"))
ri_y = ri_y[ri_y.index.isin(pac_isins)].copy()
delist_dates = {isin: extract_delist_date(ri_y.at[isin, "NAME"])
                for isin in ri_y.index}
print(f"  Delisted Pacific firms: {sum(v is not None for v in delist_dates.values())}")

# Monthly RI
ri_m_raw = load_datastream_wide(data_path("DS_RI_T_USD_M_2025.xlsx"))
keep_cols, parsed_dates = parse_monthly_columns(list(ri_m_raw.columns))
ri_m = ri_m_raw[["NAME"] + keep_cols].copy()
ri_m.columns = ["NAME"] + parsed_dates
ri_m = ri_m[ri_m.index.isin(pac_isins)].copy()
parsed_dates = [d for d in parsed_dates if d <= pd.Timestamp("2025-12-31")]
ri_m = ri_m[["NAME"] + parsed_dates]

# Monthly MV caps
mv_m_raw = load_datastream_wide(data_path("DS_MV_T_USD_M_2025.xlsx"))
keep_cols_mv, _mv = parse_monthly_columns(list(mv_m_raw.columns))
mv_m = mv_m_raw[["NAME"] + keep_cols_mv].copy()
mv_m.columns = ["NAME"] + _mv
mv_m = mv_m[mv_m.index.isin(pac_isins)].copy()
mv_m = mv_m[["NAME"] + parsed_dates].copy()
mv_m[parsed_dates] = mv_m[parsed_dates].apply(pd.to_numeric, errors="coerce")

common_isins = pac_isins & set(ri_m.index) & set(mv_m.index) & set(ri_y.index)
ri_m = ri_m.loc[list(common_isins)].copy()
mv_m = mv_m.loc[list(common_isins)].copy()
print(f"  Common ISINs: {len(common_isins)}")

# ── FIX-4: Dual RI price panels ──────────────────────────────────────────────
#   Universe panel  — December missing preserved → used for eligibility checks
#   Returns  panel  — December NOT preserved     → used for return computation
ri_prices_universe = clean_monthly_ri_prices(
    ri_m[parsed_dates], parsed_dates, LOW_PRICE_THRESHOLD,
    preserve_december_missing=True)
ri_prices_returns = clean_monthly_ri_prices(
    ri_m[parsed_dates], parsed_dates, LOW_PRICE_THRESHOLD,
    preserve_december_missing=False)

# Drop firms with no valid prices at all
all_missing = ri_prices_returns.isna().all(axis=1)
if all_missing.any():
    missing_isins      = ri_prices_returns.index[all_missing]
    ri_prices_universe = ri_prices_universe.loc[~all_missing].copy()
    ri_prices_returns  = ri_prices_returns.loc[~all_missing].copy()
    mv_m               = mv_m.drop(index=missing_isins, errors="ignore")
    print(f"  Dropped fully-missing RI rows: {all_missing.sum()}")

# Compute returns from the returns panel
ri_returns = ri_prices_returns.pct_change(axis=1, fill_method=None)

# ── FIX-2: 4-arg apply_delisting_to_returns (add ri_prices_returns) ──────────
ri_returns = apply_delisting_to_returns(
    ri_returns,
    ri_prices_returns,                                              # ← price_df
    {k: v for k, v in delist_dates.items() if k in ri_returns.index},
    parsed_dates
)

# ── FIX-3: Terminal-loss for permanent disappearances ────────────────────────
ri_returns = apply_terminal_loss_for_permanent_disappearances(
    ri_returns, ri_prices_returns, parsed_dates
)

# ── FIX-5: Adjust VW market caps for terminal-loss events ────────────────────
mv_caps_adjusted = adjust_mv_caps_for_terminal_events(
    mv_m[parsed_dates], ri_returns, parsed_dates
)
print("  Monthly returns and adjusted MV caps ready.")

# ─────────────────────────────────────────────────────────────────────────────
# STEP 2 — LOAD ANNUAL CARBON / FUNDAMENTAL DATA (forward-filled)
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Step 2 — Loading annual carbon / fundamental data…")


def ffill_annual(df: pd.DataFrame) -> pd.DataFrame:
    """Forward-fill across years per firm (leading NaN unchanged — no back-fill)."""
    return df.T.ffill().T


co2_raw     = load_annual_panel(data_path(CO2_FILE))
rev_raw     = load_annual_panel(data_path("DS_REV_Y_2025.xlsx"))
cap_ann_raw = load_annual_panel(data_path("DS_MV_T_USD_Y_2025.xlsx"))

# Revenue: dataset in thousands USD → divide by 1000 to get millions USD
rev_m   = rev_raw / 1_000.0
co2_ff  = ffill_annual(co2_raw)
revM_ff = ffill_annual(rev_m)
capA_ff = ffill_annual(cap_ann_raw)

# Firm names for top-10 table
_tmp = pd.read_excel(data_path(CO2_FILE), engine="openpyxl")
_tmp.columns = ["NAME", "ISIN"] + list(_tmp.columns[2:])
_tmp = (_tmp[~_tmp["NAME"].astype(str).str.startswith("$$ER", na=False)]
        .dropna(subset=["ISIN"]))
_tmp["ISIN"] = _tmp["ISIN"].astype(str).str.strip()
firm_names = _tmp.set_index("ISIN")["NAME"]
print("  Annual carbon data ready.")

# Risk-free rate
print(f"  Loading risk-free rate: {RF_FILE}")
try:
    rf_monthly = load_rf_monthly(data_path(RF_FILE))
    print(f"  RF: {len(rf_monthly)} obs, mean={rf_monthly.mean()*100:.3f} %/month")
except Exception as exc:
    print(f"  WARNING: RF not loaded ({exc}). Using rf=0.")
    rf_monthly = None

# ─────────────────────────────────────────────────────────────────────────────
# STEP 3 — LOAD PART 1 RETURNS & WEIGHTS
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Step 3 — Loading Part 1 portfolio returns and weights…")

ret_df = pd.read_csv(f"{RESULTS_PART1}/part1_results.csv", parse_dates=["Date"])
ret_df = ret_df.set_index("Date").sort_index()
mv_port_ret = ret_df["MV_Return"]
vw_port_ret = ret_df["VW_Return"]
print(f"  {len(mv_port_ret)} monthly observations.")

comp_df = pd.read_csv(f"{RESULTS_PART1}/part1_portfolio_compositions.csv")
comp_df["Year"] = comp_df["Year"].astype(int)
comp_df["ISIN"] = comp_df["ISIN"].astype(str).str.strip()
mv_weights_p1 = {yr - 1: grp.set_index("ISIN")["Weight"]
                 for yr, grp in comp_df.groupby("Year")}

# ─────────────────────────────────────────────────────────────────────────────
# STEP 4 — BUILD INVESTMENT SETS & Σ MATRICES
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Step 4 — Building investment sets and Σ matrices…")

inv_sets_p2   = {}
sigma_p2      = {}
vw_weights_p2 = {}
lw_deltas     = {}
lw_r_bars     = {}

for Y in DECISION_YEARS:
    # Eligibility uses the universe panel (December missing preserved)
    elig, cols = build_investment_set(
        ri_prices_universe, ri_returns, parsed_dates, Y,
        co2_panel=co2_ff
    )
    if len(elig) < 2:
        print(f"  Y={Y}: only {len(elig)} eligible firms — skipping.")
        continue

    _, Sigma_sample = estimate_moments(ri_returns, elig, cols)

    if USE_LEDOIT_WOLF:
        # FillNA with 0 before demeaning (consistent with estimate_moments approach;
        # discuss zero-fill limitation in report limitations section)
        R_mat = ri_returns.loc[elig, cols].fillna(0.0).to_numpy().T   # (T, N)
        R_dem = R_mat - R_mat.mean(axis=0, keepdims=True)
        Sigma, delta_lw, r_bar_lw = ledoit_wolf_cc(R_dem)
        lw_deltas[Y] = delta_lw
        lw_r_bars[Y] = r_bar_lw
        lw_info = f"  δ̂={delta_lw:.4f}, r̄={r_bar_lw:.4f}"
    else:
        Sigma   = Sigma_sample
        lw_info = ""

    sigma_p2[Y] = Sigma

    # Annual VW reference weights (within eligible set)
    cap_y     = (capA_ff[Y].reindex(elig).fillna(0.0).values
                 if Y in capA_ff.columns else np.zeros(len(elig)))
    total_cap = cap_y.sum()
    vw_weights_p2[Y] = (cap_y / total_cap if total_cap > 0
                        else np.ones(len(elig)) / len(elig))
    inv_sets_p2[Y] = elig
    print(f"  Y={Y}: eligible={len(elig)}{lw_info}")

# VW weight dicts in the standard format used by perf_stats_relative()
vw_weight_dicts = {Y: {"isins": inv_sets_p2[Y], "weights": vw_weights_p2[Y]}
                   for Y in inv_sets_p2}

# ─────────────────────────────────────────────────────────────────────────────
# SECTION 3.1 — CARBON INTENSITY, WACI, CARBON FOOTPRINT
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Section 3.1 — Carbon intensity, WACI, Carbon Footprint")

rows_31 = {}
for Y in DECISION_YEARS:
    if Y not in inv_sets_p2:
        continue
    isins = inv_sets_p2[Y]
    e, r, c = get_carbon_vectors(Y, isins)
    ci = carbon_intensity_vec(e, r)

    # Part 1 MV weights re-indexed to current eligible universe
    w_mv = (mv_weights_p1[Y].reindex(isins).fillna(0.0).values
            if Y in mv_weights_p1 else np.zeros(len(isins)))
    s    = w_mv.sum()
    w_mv = w_mv / s if s > 0 else np.ones(len(isins)) / len(isins)
    w_vw = vw_weights_p2[Y]

    rows_31[Y] = {
        "WACI_MV": waci_metric(w_mv, ci),
        "WACI_VW": waci_metric(w_vw, ci),
        "CF_MV"  : cf_metric(w_mv, e, c),
        "CF_VW"  : cf_vw_metric(e, c),
    }

df_31 = pd.DataFrame(rows_31).T
df_31.index.name = "Year"
df_31.to_csv(f"{RESULTS_PART2}/carbon_metrics_mv_vw.csv")
print(df_31.round(4).to_string())

# Top-10 highest-carbon firms (average CI across all decision years)
ci_acc = {}
for Y in DECISION_YEARS:
    if Y not in inv_sets_p2:
        continue
    isins = inv_sets_p2[Y]
    e, r, _ = get_carbon_vectors(Y, isins)
    for isin, ci_val in zip(isins, carbon_intensity_vec(e, r)):
        if not np.isnan(ci_val):
            ci_acc.setdefault(isin, []).append(ci_val)

mean_ci = (pd.Series({k: np.mean(v) for k, v in ci_acc.items()})
           .sort_values(ascending=False).head(10))
top10 = pd.DataFrame({"ISIN": mean_ci.index,
                      "Name": mean_ci.index.map(firm_names),
                      "Avg_CI (tCO2/M USD rev)": mean_ci.values})
top10.to_csv(f"{RESULTS_PART2}/top10_carbon_intensity.csv", index=False)
print("\nTop 10 firms by average carbon intensity:")
print(top10.to_string(index=False))

# ─────────────────────────────────────────────────────────────────────────────
# SECTION 3.2 — MV PORTFOLIO WITH 50% CF REDUCTION   P^(mv)_oos(0.5)
# min α′Σα   s.t.  CF ≤ 0.5 × CF^(MV),  sum=1, α≥0
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Section 3.2 — Min-variance with 50% Carbon Footprint constraint")

port_32        = {}
cf_32, waci_32 = {}, {}

for Y in DECISION_YEARS:
    if Y not in inv_sets_p2:
        continue
    isins     = inv_sets_p2[Y]
    Sigma     = sigma_p2[Y]
    e, r, c   = get_carbon_vectors(Y, isins)
    ec        = e_over_c_vec(e, c)
    cf_mv_Y   = df_31.loc[Y, "CF_MV"]
    if np.isnan(cf_mv_Y) or cf_mv_Y <= 0:
        print(f"  Y={Y}: CF_MV invalid — skipping 3.2.")
        continue
    cf_target = 0.5 * cf_mv_Y
    w = solve_cvxpy(Sigma, ec, cf_target, mode="mv")
    if w is None:
        print(f"  Y={Y}: CVXPY failed — falling back to Part 1 MV weights.")
        w_fb = (mv_weights_p1[Y].reindex(isins).fillna(0.0).values
                if Y in mv_weights_p1 else np.ones(len(isins)) / len(isins))
        w = np.maximum(w_fb, 0.0)
        w /= w.sum() if w.sum() > 0 else 1.0
    port_32[Y] = {"isins": isins, "weights": w}
    cf_32[Y]   = cf_metric(w, e, c)
    waci_32[Y] = waci_metric(w, carbon_intensity_vec(e, r))
    print(f"  Y={Y}  CF_target={cf_target:.3f}  achieved={cf_32[Y]:.3f}"
          f"  non-zero={int(np.sum(w > 1e-4))}/{len(isins)}")

pd.concat([pd.DataFrame({"Year": Y, "ISIN": v["isins"], "Weight": v["weights"]})
           for Y, v in port_32.items()]).to_csv(
    f"{RESULTS_PART2}/weights_32_mv_carbon05.csv", index=False)
ret_32 = compute_oos_returns(port_32, ri_returns, parsed_dates,
                              START_YEAR_OOS, END_YEAR_OOS)
ret_32.to_csv(f"{RESULTS_PART2}/returns_32_mv_carbon05.csv", header=["Return"])

stats_32 = pd.DataFrame({
    "P_mv_oos"     : perf_stats_extended(mv_port_ret, rf=rf_monthly),
    "P_mv_oos(0.5)": perf_stats_extended(ret_32,      rf=rf_monthly),
}).T
stats_32.to_csv(f"{RESULTS_PART2}/stats_32_comparison.csv")
print("\nSection 3.2 performance:")
print(stats_32.round(4).to_string())

# ─────────────────────────────────────────────────────────────────────────────
# SECTION 3.3 — TE MINIMISATION WITH 50% CF REDUCTION  P^(vw)_oos(0.5)
# min (α−α_vw)′Σ(α−α_vw)  s.t.  CF ≤ 0.5×CF^(VW),  sum=1, α≥0
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Section 3.3 — TE minimisation with 50% Carbon Footprint constraint")

port_33        = {}
cf_33, waci_33 = {}, {}

for Y in DECISION_YEARS:
    if Y not in inv_sets_p2:
        continue
    isins     = inv_sets_p2[Y]
    Sigma     = sigma_p2[Y]
    e, r, c   = get_carbon_vectors(Y, isins)
    ec        = e_over_c_vec(e, c)
    w_vw      = vw_weights_p2[Y]
    cf_vw_Y   = df_31.loc[Y, "CF_VW"]
    if np.isnan(cf_vw_Y) or cf_vw_Y <= 0:
        print(f"  Y={Y}: CF_VW invalid — skipping 3.3.")
        continue
    cf_target = 0.5 * cf_vw_Y
    w = solve_cvxpy(Sigma, ec, cf_target, w_ref=w_vw, mode="te")
    if w is None:
        print(f"  Y={Y}: CVXPY failed — falling back to VW weights.")
        w = w_vw.copy()
    port_33[Y] = {"isins": isins, "weights": w}
    cf_33[Y]   = cf_metric(w, e, c)
    waci_33[Y] = waci_metric(w, carbon_intensity_vec(e, r))
    print(f"  Y={Y}  CF_target={cf_target:.3f}  achieved={cf_33[Y]:.3f}"
          f"  non-zero={int(np.sum(w > 1e-4))}/{len(isins)}")

pd.concat([pd.DataFrame({"Year": Y, "ISIN": v["isins"], "Weight": v["weights"]})
           for Y, v in port_33.items()]).to_csv(
    f"{RESULTS_PART2}/weights_33_te_carbon05.csv", index=False)
ret_33 = compute_oos_returns(port_33, ri_returns, parsed_dates,
                              START_YEAR_OOS, END_YEAR_OOS)
ret_33.to_csv(f"{RESULTS_PART2}/returns_33_te_carbon05.csv", header=["Return"])

stats_33 = pd.DataFrame({
    "P_vw_oos"     : perf_stats_extended(vw_port_ret, rf=rf_monthly),
    "P_vw_oos(0.5)": perf_stats_extended(ret_33,      rf=rf_monthly),
}).T
stats_33.to_csv(f"{RESULTS_PART2}/stats_33_comparison.csv")
print("\nSection 3.3 performance:")
print(stats_33.round(4).to_string())

# ─────────────────────────────────────────────────────────────────────────────
# SECTION 4.1 — NET-ZERO PORTFOLIO   P^(vw)_oos(NZ)
# min (α−α_vw)′Σ(α−α_vw)
# s.t. CF ≤ (1−θ)^{Y−Y0+1} × CF^(VW)_{Y0},  sum=1, α≥0
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print(f"Section 4.1 — Net-zero portfolio (θ={THETA:.0%}/yr from Y0={Y0})")

# Reference: VW carbon footprint at Y0 = 2013
isins_y0 = inv_sets_p2.get(Y0, [])
if isins_y0:
    e0, _, c0 = get_carbon_vectors(Y0, isins_y0)
    cf_vw_y0  = cf_vw_metric(e0, c0)
else:
    cf_vw_y0  = df_31.loc[Y0, "CF_VW"] if Y0 in df_31.index else np.nan
print(f"  CF_vw reference Y0={Y0}: {cf_vw_y0:.4f} tCO2/M USD")

port_41        = {}
cf_41, waci_41 = {}, {}

for Y in DECISION_YEARS:
    if Y not in inv_sets_p2:
        continue
    isins     = inv_sets_p2[Y]
    Sigma     = sigma_p2[Y]
    e, r, c   = get_carbon_vectors(Y, isins)
    ec        = e_over_c_vec(e, c)
    w_vw      = vw_weights_p2[Y]
    cf_target = ((1.0 - THETA) ** (Y - Y0 + 1)) * cf_vw_y0
    w = solve_cvxpy(Sigma, ec, cf_target, w_ref=w_vw, mode="te")
    if w is None:
        print(f"  Y={Y}: CVXPY failed — falling back to VW weights.")
        w = w_vw.copy()
    port_41[Y] = {"isins": isins, "weights": w}
    cf_41[Y]   = cf_metric(w, e, c)
    waci_41[Y] = waci_metric(w, carbon_intensity_vec(e, r))
    print(f"  Y={Y}  CF_target={cf_target:.4f}  achieved={cf_41[Y]:.4f}"
          f"  non-zero={int(np.sum(w > 1e-4))}/{len(isins)}")

pd.concat([pd.DataFrame({"Year": Y, "ISIN": v["isins"], "Weight": v["weights"]})
           for Y, v in port_41.items()]).to_csv(
    f"{RESULTS_PART2}/weights_41_netzero.csv", index=False)
ret_41 = compute_oos_returns(port_41, ri_returns, parsed_dates,
                              START_YEAR_OOS, END_YEAR_OOS)
ret_41.to_csv(f"{RESULTS_PART2}/returns_41_netzero.csv", header=["Return"])

stats_41 = pd.DataFrame({
    "P_vw_oos"     : perf_stats_extended(vw_port_ret, rf=rf_monthly),
    "P_vw_oos(0.5)": perf_stats_extended(ret_33,      rf=rf_monthly),
    "P_vw_oos(NZ)" : perf_stats_extended(ret_41,      rf=rf_monthly),
}).T
stats_41.to_csv(f"{RESULTS_PART2}/stats_41_comparison.csv")
print("\nSection 4.1 — three-way comparison:")
print(stats_41.round(4).to_string())

# ─────────────────────────────────────────────────────────────────────────────
# TOTAL ATTRIBUTED CO2 IN TONNES
# Brief §3.1: "assuming starting wealth V_2013 = $1 M"
# TotalCO2_Y = V_Y × CF^(p)_Y  (CF is in tCO2 per M USD invested)
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
print("Total attributed CO2 (V_2013 = $1 M starting wealth)…")

V0_M = 1.0   # million USD


def annual_portfolio_values(monthly_ret: pd.Series,
                             v0: float = 1.0) -> pd.Series:
    """
    Compound monthly returns year-by-year to track end-of-year wealth.
    V[Y] = wealth at end of decision year Y (= start of investment year Y+1).
    V[Y0] = v0 (initial wealth, no returns yet).
    """
    values = {Y0: v0}
    v = v0
    for year in range(START_YEAR_OOS, END_YEAR_OOS + 1):
        yr_rets = monthly_ret[monthly_ret.index.year == year]
        if len(yr_rets) > 0:
            v = v * (1.0 + yr_rets).prod()
        values[year - 1] = v    # wealth available at end of year-1
    values[Y0] = v0
    return pd.Series(values).sort_index().loc[DECISION_YEARS]


v_mv = annual_portfolio_values(mv_port_ret, V0_M)
v_32 = annual_portfolio_values(ret_32,      V0_M)
v_33 = annual_portfolio_values(ret_33,      V0_M)
v_41 = annual_portfolio_values(ret_41,      V0_M)

cf_mv_s = df_31["CF_MV"]
cf_vw_s = df_31["CF_VW"]
cf_32_s = pd.Series(cf_32)
cf_33_s = pd.Series(cf_33)
cf_41_s = pd.Series(cf_41)

total_co2 = pd.DataFrame({
    "TotalCO2_MV"  : v_mv * cf_mv_s,
    "TotalCO2_VW"  : v_mv * cf_vw_s,
    "TotalCO2_MV05": v_32 * cf_32_s,
    "TotalCO2_TE05": v_33 * cf_33_s,
    "TotalCO2_NZ"  : v_41 * cf_41_s,
    "V_MV_MUSD"    : v_mv,
    "V_MV05_MUSD"  : v_32,
    "V_TE05_MUSD"  : v_33,
    "V_NZ_MUSD"    : v_41,
}).round(4)
total_co2.index.name = "Year"
total_co2.to_csv(f"{RESULTS_PART2}/total_attributed_co2.csv")
print(total_co2[["TotalCO2_MV", "TotalCO2_VW",
                  "TotalCO2_MV05", "TotalCO2_TE05", "TotalCO2_NZ"]].round(1)
      .to_string())

# ─────────────────────────────────────────────────────────────────────────────
# MASTER SUMMARY FILES  (ADD-3 / ADD-4 / ADD-5)
# ─────────────────────────────────────────────────────────────────────────────

# ── Extended stats for all 5 portfolios  (ADD-3) ─────────────────────────────
all_stats = pd.DataFrame({
    "P_mv_oos"     : perf_stats_extended(mv_port_ret, rf=rf_monthly),
    "P_mv_oos(0.5)": perf_stats_extended(ret_32,      rf=rf_monthly),
    "P_vw_oos"     : perf_stats_extended(vw_port_ret, rf=rf_monthly),
    "P_vw_oos(0.5)": perf_stats_extended(ret_33,      rf=rf_monthly),
    "P_vw_oos(NZ)" : perf_stats_extended(ret_41,      rf=rf_monthly),
}).T
all_stats.to_csv(f"{RESULTS_PART2}/all_portfolio_stats_extended.csv")
print("\n=== ALL PORTFOLIO STATISTICS (EXTENDED) ===")
print(all_stats.round(4).to_string())

# ── Benchmark-relative stats for TE portfolios  (ADD-4) ──────────────────────
rel_stats = pd.DataFrame({
    "P_vw_oos(0.5)": perf_stats_relative(
        ret_33, vw_port_ret, port_33, vw_weight_dicts, rf=rf_monthly),
    "P_vw_oos(NZ)" : perf_stats_relative(
        ret_41, vw_port_ret, port_41, vw_weight_dicts, rf=rf_monthly),
}).T
rel_stats.to_csv(f"{RESULTS_PART2}/relative_stats_te_portfolios.csv")
print("\n=== BENCHMARK-RELATIVE STATS (TE portfolios) ===")
print(rel_stats.round(4).to_string())

# ── Carbon metrics + LW parameters ───────────────────────────────────────────
carbon_df = pd.DataFrame({
    "CF_MV"     : cf_mv_s,  "CF_VW"     : cf_vw_s,
    "CF_MV_05"  : cf_32_s,  "CF_TE_05"  : cf_33_s,  "CF_NZ"     : cf_41_s,
    "WACI_MV"   : df_31["WACI_MV"],    "WACI_VW"   : df_31["WACI_VW"],
    "WACI_MV_05": pd.Series(waci_32),  "WACI_TE_05": pd.Series(waci_33),
    "WACI_NZ"   : pd.Series(waci_41),
})
if USE_LEDOIT_WOLF:
    carbon_df["LW_delta"] = pd.Series(lw_deltas)
    carbon_df["LW_r_bar"] = pd.Series(lw_r_bars)
carbon_df.rename_axis("Year").to_csv(f"{RESULTS_PART2}/all_carbon_metrics.csv")

# ── Net-zero target path ──────────────────────────────────────────────────────
nz_path = pd.Series(
    {Y: ((1.0 - THETA) ** (Y - Y0 + 1)) * cf_vw_y0 for Y in DECISION_YEARS},
    name="NZ_target"
).rename_axis("Year")
nz_path.to_csv(f"{RESULTS_PART2}/nz_target_path.csv")

# ── Section-level tables for copy-paste into report  (ADD-5) ─────────────────
pd.DataFrame({
    "P_mv_oos"     : perf_stats_extended(mv_port_ret, rf=rf_monthly),
    "P_mv_oos(0.5)": perf_stats_extended(ret_32,      rf=rf_monthly),
    "P_vw_oos"     : perf_stats_extended(vw_port_ret, rf=rf_monthly),
    "P_vw_oos(0.5)": perf_stats_extended(ret_33,      rf=rf_monthly),
}).T.to_csv(f"{RESULTS_PART2}/stats_34_section_comparison.csv")

pd.DataFrame({
    "P_vw_oos"     : perf_stats_extended(vw_port_ret, rf=rf_monthly),
    "P_vw_oos(0.5)": perf_stats_extended(ret_33,      rf=rf_monthly),
    "P_vw_oos(NZ)" : perf_stats_extended(ret_41,      rf=rf_monthly),
}).T.to_csv(f"{RESULTS_PART2}/stats_42_section_comparison.csv")

print("=" * 60)
print(f"All Part 2 results saved to '{{RESULTS_PART2}}/'")
for fn in sorted(os.listdir(RESULTS_PART2)):
    print(f"  {{fn}}")
print("=" * 60)
