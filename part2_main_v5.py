"""
SAAM Project — Part 2  (v5 — three-bug fix release)
Sections 3.1 | 3.2 | 3.3 | 3.4 | 4.1 | 4.2  —  Group: Pacific / Scope 1

Fixes vs v4
───────────
Bug 1 (CRITICAL, Part 1 side): Part 1 now forward-fills the CO2 panel before
  passing it to build_investment_set().  Part 2 already did this via co2_ff.
  Both parts therefore use the same forward-filled panel → identical investment
  universes.  No code change needed in this file; the fix is in Part 1 v4.

Bug 2 (IMPORTANT, shared): estimate_moments() now uses ddof=0 matching the
  brief's Σ_Y = (1/τ) Σ (R_t − μ̂)(R_t − μ̂)'.  No code change needed here;
  the fix is in Part 1 v4 which this file imports.

Bug 3 (MINOR, this file): dead / broken one-liner for cf_vw_y0 in Section 4.1
  has been removed.  Only the clean block remains.

Retained from v4
────────────────
• Imports build_investment_set, estimate_moments, perf_stats, load_rf_monthly
  from saam_part1_complete_v4.py (zero duplication).
• build_investment_set() called with co2_panel=co2_ff everywhere.
• carbon_eligible_isins() removed (redundant since carbon filter is inside
  build_investment_set).
• All perf_stats() calls pass rf=rf_monthly (Sharpe with actual RF).
• Ledoit-Wolf shrinkage; fillna(0.0) only for the LW matrix algebra (see note).

Dependencies: cvxpy, pandas, numpy, openpyxl
Run: python part2_main_v5.py
"""
...
build_investment_set       = _p1.build_investment_set   # includes CO2 filter
estimate_moments           = _p1.estimate_moments       # NaN-aware, ddof=0
perf_stats                 = _p1.perf_stats             # accepts rf= parameter
...
rev_m   = rev_raw / 1_000.0          # thousands USD → millions USD
co2_ff  = ffill_annual(co2_raw)      # used for carbon filter AND metrics
...
# Note on fillna(0.0): the LW estimator requires a complete (T×N) demeaned
# matrix for its vectorised algebra.  Unlike estimate_moments() which uses
# pairwise deletion (no fillna), here fillna(0.0) is applied after loading
# returns to build the matrix — a necessary numerical step distinct from the
# estimate_moments() bug.  The investment-set filters ensure residual NaNs
# are only leading/trailing listing-period gaps, so the induced bias is small.
# ─────────────────────────────────────────────────────────────────────────────
...
# build_investment_set() called with co2_panel=co2_ff (forward-filled).
# Carbon filter lives inside Part 1's function — same universe for both parts.
# ─────────────────────────────────────────────────────────────────────────────
print("=" * 60)
...
for Y in DECISION_YEARS:
    elig, cols = build_investment_set(
        ...
        co2_panel=co2_ff           # forward-filled, same as Part 1 v4
    )
    if len(elig) < 2:
        print(f"  Y={Y}: only {len(elig)} eligible firms — skipping.")
        continue

    _, Sigma_sample = estimate_moments(ri_returns, elig, cols)  # ddof=0

    if USE_LEDOIT_WOLF:
        # fillna(0.0) needed here for complete (T×N) LW matrix — see note above
...
# BUG 3 FIX: removed the dead one-liner that called get_carbon_vectors() twice
# and was immediately overwritten.  Only the clean block below remains.
...
pd.DataFrame({
    "CF_MV"    : df_31["CF_MV"],     "CF_VW"    : df_31["CF_VW"],
    "CF_MV_05" : pd.Series(cf_32),   "CF_TE_05" : pd.Series(cf_33),
...
pd.Series(
    ...
    name="NZ_target"
).rename_axis("Year").to_csv(f"{RESULTS_PART2}/nz_target_path.csv")
