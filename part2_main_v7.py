"""
SAAM Project — Part 2 (v7 — persists LW parameters for Fig 8)
Sections 3.1 | 3.2 | 3.3 | 3.4 | 4.1 | 4.2 — Group: Pacific / Scope 1

Changes vs v6
─────────────
FIX — LW shrinkage parameters (δ̂, r̄) are now stored per decision year
      and written as columns LW_delta / LW_r_bar into all_carbon_metrics.csv.
      This enables Fig 8 in part2_plots_v3.py (condition was always True
      for USE_LEDOIT_WOLF, but "LW_delta" in carbon.columns silently failed).

All previous fixes (v4/v5/v6) retained:
...
v6     — total attributed CO2 in tonnes (TotalCO2 = V_Y × CF_Y).
"""
...
def track_portfolio_value(monthly_returns: pd.Series,
    ...
    Returns a pd.Series indexed by DECISION YEAR Y (2013…2024):
...
sigma_p2      = {}
vw_weights_p2 = {}
lw_deltas     = {}   # ← NEW: store δ̂ per year
lw_r_bars     = {}   # ← NEW: store r̄ per year
...
for Y in DECISION_YEARS:
    ...
    if USE_LEDOIT_WOLF:
        ...
        lw_deltas[Y] = delta_lw   # ← NEW
        lw_r_bars[Y] = r_bar_lw   # ← NEW
    else:
        Sigma = Sigma_sample
...
def annual_portfolio_values(monthly_ret: pd.Series, v0: float = 1.0) -> pd.Series:
    ...
    Returns Series indexed by decision year Y = investment_year - 1,
    giving wealth at END of year Y (= start of investment year Y+1).
    V[2013] = v0 (no investment has happened yet).
    """
    values = {}
    v = v0
    for year in range(START_YEAR_OOS, END_YEAR_OOS + 1):
...
# ── all_carbon_metrics.csv — now includes LW_delta and LW_r_bar ──────────────
...
if USE_LEDOIT_WOLF:                          # ← NEW: persist LW parameters
