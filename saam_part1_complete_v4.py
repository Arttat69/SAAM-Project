...
def estimate_moments(ri_returns, isins, cols):
    ...
    NaN-aware approach (Arthur's fix, retained from v3)
    ─────────────────────────────────────────────────────
    Missing returns are NEVER replaced by zero (no fillna(0.0)).
    Imputing zero biases mu toward zero for firms with partial histories and
    artificially reduces variance / covariance.

    mu:    arithmetic mean of each firm's OBSERVED monthly returns
           (pandas mean skipna=True → divides by number of non-NaN obs).

    Sigma: pairwise-deletion covariance with ddof=0  ← Bug 2 fix
           Each (i,j) entry uses only months where BOTH firms have
           observed returns, dividing by T_ij (the brief's formula
           Σ_Y = (1/τ) Σ_t (R_t − μ̂)(R_t − μ̂)' uses τ in the
           denominator, not τ−1; ddof=0 enforces this convention).
           Pairs with zero common months are set to 0.
    """
    R     = ri_returns.loc[isins, cols].astype(float)
    mu    = R.mean(axis=1, skipna=True).to_numpy()

    # ddof=0: divides by T_ij, matching the brief's (1/τ) formula  [Bug 2 fix]
    Sigma = R.T.cov(ddof=0).to_numpy()
...
def compute_vw_oos_returns(ri_returns, mv_caps, dates,
                            start_year=2014, end_year=2025):
    out_r, out_d = [], []
    months       = [d for d in dates
                    if pd.Timestamp(f"{start_year}-01-01") <= d
...
def main():
    ...
    vw_r = compute_vw_oos_returns(
        ri_returns, mv_m[parsed_dates], parsed_dates,
