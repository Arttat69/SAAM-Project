...
# ─────────────────────────────────────────────────────────────────────────────
# COVARIANCE ESTIMATOR OPTION
# ─────────────────────────────────────────────────────────────────────────────
# Option 1 (USE_LEDOIT_WOLF = False): sample covariance + tiny ridge (as in Part 1)
# Option 2 (USE_LEDOIT_WOLF = True) : Ledoit-Wolf (2004) constant-correlation
#   shrinkage estimator, implemented from scratch following Lecture 4 formulas.
#   Fixes the rank-deficiency issue (N > T) that causes sections 3.3/4.1 to
#   produce identical results with the plain sample covariance matrix.
USE_LEDOIT_WOLF = True    # ← flip to False to run without shrinkage
RESULTS_PART2   = "ResultsPart2_LW" if USE_LEDOIT_WOLF else "ResultsPart2"
os.makedirs(RESULTS_PART2, exist_ok=True)  # folder set by USE_LEDOIT_WOLF flag
...
#             Matrix", Journal of Portfolio Management, 110-119.
# Lecture   : SAAM Lecture 4 — Regularization of Covariance Matrix.
#
# The estimator is:
#   Σ̂_shrink = δ̂ · F  +  (1 − δ̂) · S
#
# where
#   S   = sample covariance matrix  (unbiased, but large estimation error when N>T)
#   F   = constant-correlation target  (biased but structured / low estimation error)
#           f_ii  = s_ii  (variances unchanged)
#           f_ij  = r̄ · √(s_ii · s_jj)  for i≠j   (all correlations set to r̄)
#           r̄    = average of all sample pairwise correlations
#   δ̂  = optimal shrinkage intensity (Frobenius-norm-optimal, data-driven):
#           δ̂ = max(0, min(1, κ̂/T))
#           κ̂ = (π̂ − ρ̂) / γ̂
#           π̂_ij = (1/T) Σ_t [(x_{i,t}·x_{j,t} − s_ij)²]   ← asymptotic Var of s_ij × T
#           π̂    = Σ_ij π̂_ij
#           θ̂_{ii,ij} = (1/T) Σ_t [(x_{i,t}²−s_ii)(x_{i,t}x_{j,t}−s_ij)]
#           θ̂_{jj,ij} = (1/T) Σ_t [(x_{j,t}²−s_jj)(x_{i,t}x_{j,t}−s_ij)]
#           ρ̂    = Σ_i π̂_ii + Σ_{i≠j} (r̄/2)[√(s_jj/s_ii)·θ̂_{ii,ij} + √(s_ii/s_jj)·θ̂_{jj,ij}]
#           γ̂    = Σ_ij (f_ij − s_ij)²   ← misspecification of target
#
# All computations are fully vectorised (no Python loops).
# ─────────────────────────────────────────────────────────────────────────────
...
def ledoit_wolf_cc(X: np.ndarray):
    ...
    X : np.ndarray, shape (T, N)
        Matrix of DEMEANED returns (rows = observations, cols = assets).

    Returns
    -------
    Sigma_shrunk : np.ndarray (N, N)  — regularised covariance matrix
    delta        : float              — optimal shrinkage intensity δ̂
    r_bar        : float              — average sample correlation r̄
    """
    T, N = X.shape

    # ── Sample covariance matrix S ────────────────────────────────────────────
    S   = X.T @ X / T                           # (N, N)
    var = np.diag(S)                            # (N,)  sample variances
    std = np.sqrt(np.maximum(var, 1e-16))       # (N,)  guard against zero-var

    # ── Average sample correlation r̄ ─────────────────────────────────────────
    std_outer = np.outer(std, std)              # (N, N)
    corr      = S / std_outer                   # (N, N) full correlation matrix
    np.fill_diagonal(corr, 0.0)
    r_bar = corr.sum() / (N * (N - 1))         # scalar: mean off-diagonal correlation

    # ── Constant-correlation target F ────────────────────────────────────────
    # f_ii = s_ii,  f_ij = r̄ · √(s_ii · s_jj)
    F = r_bar * std_outer                       # (N, N)
    np.fill_diagonal(F, var)                    # restore exact sample variances

    # ── π̂: asymptotic variance of the sample covariance entries ─────────────
    # π̂_ij = (1/T) Σ_t (x_{i,t}·x_{j,t} − s_ij)²
    #       = mean(x_i² · x_j²) − s_ij²
    X2     = X ** 2                             # (T, N)
    pi_mat = X2.T @ X2 / T - S ** 2            # (N, N)
    pi_hat = pi_mat.sum()

    # ── ρ̂: asymptotic covariance between F and S entries ────────────────────
    # Diagonal contribution: ρ̂_ii = π̂_ii (since F_ii = S_ii exactly)
    rho_hat = np.diag(pi_mat).sum()

    # Off-diagonal contribution
    # θ̂_ii,ij = (1/T) Σ_t (x_{i,t}²−s_ii)(x_{i,t}·x_{j,t}−s_ij)
    #          = mean(x_i³·x_j) − s_ii·s_ij
    # θ̂_jj,ij = (1/T) Σ_t (x_{j,t}²−s_jj)(x_{i,t}·x_{j,t}−s_ij)
    #          = mean(x_i·x_j³) − s_jj·s_ij
    X3       = X ** 3                           # (T, N)
    Theta_II = X3.T @ X / T - S * var[:, None] # (N,N): [i,j] = θ̂_{ii,ij}
    Theta_JJ = X.T @ X3 / T - S * var[None, :] # (N,N): [i,j] = θ̂_{jj,ij}

    # ρ̂_ij = (r̄/2) · [√(s_jj/s_ii)·θ̂_{ii,ij} + √(s_ii/s_jj)·θ̂_{jj,ij}]
    ratio_JI = std[None, :] / std[:, None]      # (N,N): std_j / std_i
    ratio_IJ = std[:, None] / std[None, :]      # (N,N): std_i / std_j
    rho_mat  = (r_bar / 2) * (ratio_JI * Theta_II + ratio_IJ * Theta_JJ)

    # Sum off-diagonal elements only (i ≠ j)
    rho_hat += rho_mat.sum() - np.diag(rho_mat).sum()

    # ── γ̂: misspecification of the shrinkage target ──────────────────────────
    gamma_hat = np.sum((F - S) ** 2)

    # ── Optimal shrinkage intensity ───────────────────────────────────────────
    # δ̂ = max(0, min(1, κ̂/T))  with  κ̂ = (π̂ − ρ̂) / γ̂
    ...
    return Sigma_shrunk, delta, r_bar
...
for Y in DECISION_YEARS:
    ...
    _, Sigma_sample = estimate_moments(ri_returns, eligible, cols)

    if USE_LEDOIT_WOLF:
        # Build demeaned (T x N) return matrix for LW estimator
        R_mat     = ri_returns.loc[eligible, cols].fillna(0.0).to_numpy().T  # (T, N)
        R_demean  = R_mat - R_mat.mean(axis=0, keepdims=True)
        Sigma, delta_lw, r_bar_lw = ledoit_wolf_cc(R_demean)
    else:
        Sigma = Sigma_sample   # plain sample covariance + ridge (Part 1 default)
    ...
    print(f"  Y={Y}: eligible={len(eligible)}/{len(base_isins)}{lw_info}")
